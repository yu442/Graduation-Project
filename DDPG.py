...
class Actor(object):
    ...
    def _build_net(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ...
            net1 = tf.layers.dense(s, 256, activation=tf.nn.relu,
            ...
            with tf.variable_scope('a'):
                actions = tf.layers.dense(net3, self.a_dim, activation=tf.nn.tanh, kernel_initializer=init_w,
    ...
    def _build_net(self, s, a, scope, trainable):
        with tf.variable_scope(scope):
            ...
            with tf.variable_scope('q'):
                q = tf.layers.dense(net3, 1, kernel_initializer=init_w, bias_initializer=init_b, trainable=trainable)   # Q(s,a)
        return q
...
for i in range(MAX_EPISODES):
    ...
    for j in range(MAX_EP_STEPS):
        ...
        if j == MAX_EP_STEPS-1 or done:
            print('Episode:', i, ' Reward: %i' % int(ep_reward/j), 'Explore: %.2f' % var, 'step: ',j,'speed',s[1],Env.vl,Env
                  .max)
            break
    if i % 10 == 0:
